    ...
    public class PostProcessPass : ScriptableRenderPass
    {
        ...
        void Render(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            int GetDestination()
            {
                ...
                else if (destination == m_Source.id && (m_Descriptor.msaaSamples > 1 || isFXAA))
                {
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(URPProfileId.UberPostProcess)))
            {
...
#endif
                {
                    ...
                    if (isFXAA)
                    {
                        ...
                        {
                            Swap();
                            DoFastApproximateAntialiasing(cmd, ref renderingData, GetSource(), m_Source.id);
                        }
                    }
        ...
        void DoFastApproximateAntialiasing(CommandBuffer cmd, ref RenderingData renderingData, int source, int destination)
        {
            ref var cameraData = ref renderingData.cameraData;
            var material = m_Materials.finalPass;
            material.shaderKeywords = null;
